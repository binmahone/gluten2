...
class BasicWriteTaskStatsTracker(
  ...
  /**
   * Get the size of the file expected to have been written by a worker. This supports the XAttr in
   * HADOOP-17414 when the "magic committer" adds a custom HTTP header to the a zero byte marker. If
   * the output file as returned by getFileStatus > 0 then the length if returned. For zero-byte
   * files, the (optional) Hadoop FS API getXAttr() is invoked. If a parseable, non-negative length
   * can be retrieved, this is returned instead of the length.
   * @return
   *   the file size or None if the file was not found.
   */
