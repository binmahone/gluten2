...
class ParquetFileFormat extends FileFormat with DataSourceRegister with Logging with Serializable {
  ...
  override def prepareWrite(
    ...
    if (GlutenConfig.getConf.enableGluten && GlutenConfig.getConf.enableNativeParquetWriter) {
      GlutenParquetWriterInjects.getInstance().createOutputWriterFactory
    } else {
  ...
  override def inferSchema(
    ...
    if (
      GlutenConfig.getConf.enableGluten && GlutenConfig.getConf.enableNativeParquetWriter
      && GlutenConfig.isCurrentBackendVelox
    ) {
      GlutenParquetWriterInjects.getInstance().inferSchema
