...
class GlutenClickHouseWriteParquetTableSuite()
    ...
    withSQLConf(("spark.gluten.sql.native.parquet.writer.enabled", "true")) {
      ...
      val parquet_table_create_sql =
        s"create table if not exists $parquet_table_name (" +
          ...
          "stored as parquet"
      // spark write does not support bucketing for hive table
  ...
  test("test hive parquet table, all columns being partitioned") {
    withSQLConf(
      ...
      // TODO: what will happen if user specify a decimal partition col?
      val parquet_table_create_sql =
        s"create table if not exists $parquet_table_name (" +
          " decimal_field decimal(23,12)" + " ) partitioned by (" +
          fields
            .filterNot(e => e._1.equals("decimal_field"))
            .map(f => s"${f._1} ${f._2}")
            .mkString(",") +
          ") " +
          "stored as parquet"
      ...
      writeAndCheckRead(
        fields => {
          spark
            ...
            .write
            .partitionBy("date_field")
            .bucketBy(2, "byte_field", "string_field")
            .saveAsTable(parquet_table_name)
        },
        fields.keys.toSeq
      )
